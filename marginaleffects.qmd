...
- ここに、StressEchoのデータセットの説明
```{r}
stressEcho <- data.table::fread("stressEcho.csv")

dd <- rms::datadist(stressEcho)
options("datadist" = dd)
...
hookaddcap()   # make knitr call a function at the end of each chunk
               # to try to automdatically add to list of figure

Hmisc::describe(stressEcho)



```

- ここで、Max HRを予測するolsを作る
- しかし、ここで上級医のつっこみ
