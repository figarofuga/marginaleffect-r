...
spline_fit <- glm(formula =  death_01 ~ swang1*cat_chf + rcs(age, 4) * rcs(crea1, 4) + sex + race + edu + income + wtkilo1 + temp1 + meanbp1 + resp1 + hrt1 + pafi1 + paco21 + ph1 + wblc1 + hema1 + sod1 + pot1 + bili1 + alb1 + cardiohx + chfhx + immunhx + transhx + amihx,
               family = binomial,
               data    = rhc_prep)
...
spline_fig1 <- marginaleffects::plot_predictions(spline_fit, condition = "age", type = "response") + 
...
spline_dat2 <- marginaleffects::predictions(
  model = spline_fit, 
  newdata = datagrid(age = seq(0, 100, by = 2), crea1 = seq(0, 10, 0.2)))

plot_ly(spline_dat2, x = ~age, y = ~crea1, z = ~estimate, opacity = 0.5) |>
...
::: {.column width="50%"}
```{r}
marginaleffects::predictions(spline_fit) |> 
  as.data.frame() |> 
  dplyr::select(rowid, estimate, p.value, conf.low, conf.high, death_01, age, sex, swang1) |> 
  tibble::as_tibble()
...
::: {.column width="50%"}
```{r}
marginaleffects::predictions(spline_fit, variables = "swang1") |> 
  as.data.frame() |> 
  dplyr::select(rowid, estimate, p.value, conf.low, conf.high, death_01, age, sex, swang1) |> 
  tibble::as_tibble()
