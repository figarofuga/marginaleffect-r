...
- 通常のアウトカム式のみで一発勝負
- 一応Biasを避けた上でのBias補正方法も存在する[@grf-lab2024-jt]
...
1. まずは、1年後のAFEQT-OSを予測するOLS modelを作成
  1. Interventionの列を独立因子に入れておく(Rate, AAD, CA)
  1. この時、BaselineのAFEQT-OSをSplineにしたり、各症状とInterventionでInteraction項を作成
2. 全てのデータで、患者を3倍に拡張して(Interventionを全て"Rate", "AAD", "CA")にする
3. 予測モデルを利用して、各患者でのPredicted AFEQT-OS score列を作成
4. 症状がある患者だけを抽出して、平均、95%信頼区間を作成
