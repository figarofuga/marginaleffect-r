...
spline_fit <- lrm(death_01 ~ rcs(age, 4) + rcs(crea1, 4) + cat1 + sex + meanbp1 + hrt1 + resp1 + alb1, data = rhc_prep)

spline_dat <- Predict(spline_fit, age = seq(0, 100, by = 2), crea1 = seq(0, 10, 0.2)) |> 
  dplyr::rename(`Odds ratio` = yhat, 
                Cr = crea1, 
                Age = age)

plot_ly(spline_dat, x = ~Age, y = ~Cr, z = ~`Odds ratio`) |>
