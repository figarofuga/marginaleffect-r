...
spline_fit <- lrm(death_01 ~ rcs(age, 4) + crea1 + cat1 + sex + meanbp1 + hrt1 + resp1 + alb1, data = rhc_prep)

spline_fig1 <- Predict(spline_fit, age, fun = exp) |> 
  ggplot()

spline_fit2 <- lrm(death_01 ~ rcs(age, 4) * rcs(crea1, 4) + cat1 + sex + meanbp1 + hrt1 + resp1 + alb1, data = rhc_prep)
...
plot_ly(spline_dat2, x = ~Age, y = ~Cr, z = ~`Odds ratio`) |>
...
-   通常は線形として扱う
-   しかし、連続値であっても線形である事は少ない - BMIや血圧はJ曲線
-   Log？exponentiate？累乗？
